      ...
      <main class="h-full overflow-y-auto px-4 py-4 bg-surface flex flex-col gap-4">
        ...
        <div class="bg-white border border-border rounded-lg shadow-xs flex flex-col">
          ...
          <div class="grid grid-cols-[minmax(240px,2fr)_140px_160px_1fr_220px] border-b border-border shrink-0">
            ...
            <div class="px-6 py-3 text-xs font-medium text-text-tertiary leading-[18px] text-right"></div>
          </div>
          ...
          >
            ...
            <div class="px-4 py-2 flex items-center justify-end gap-0.5">
              <HoverTooltip label="Copier le code">
                <button
                  type="button"
              ...
              <HoverTooltip v-if="agent.phone" label="Partager par WhatsApp">
                <button
                  type="button"
              ...
              <HoverTooltip label="Régénérer le code">
                <button
                  type="button"
                  class="p-2 rounded-lg text-text-quaternary hover:text-text-secondary hover:bg-surface transition-colors disabled:opacity-40 disabled:hover:bg-transparent"
                  :disabled="agent.statut === 'inactif'"
...
function copyPin(agent: DataosAgent) {
  ...
  toast.show({
    ...
    description: `Le code de ${agent.name} est dans le presse-papiers.`,
  })
}
...
function onAgentCreated(agent: DataosAgent) {
  ...
  if (agent.phone) {
    actions.push({ label: 'Partager par WhatsApp', onClick: () => shareWhatsApp(agent) })
  }
  toast.show({
    title: 'Agent ajouté',
    description: `Code PIN ${agent.pin} attribué à ${agent.name}. Partage-le pour qu'il puisse se connecter.`,
    actions,
    duration: 0,
  })
}
