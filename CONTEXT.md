...
**Collaborateur**:
A [[Membre]] of type **web** — backed by a [[User]] (personal account: email, password). Logs into the **web** platform, holds one [[Rôle]] (natif or custom) that determines its [[Accès (module)]], works in the back-office (configures workflows, imports, observes). Identified by **email**, the login identifiant: set at invitation and **immutable** afterwards. The edit panel greys the email field out; to change it, remove the Collaborateur and re-invite. The same [[User]] is a Collaborateur of each Organisation they belong to. One of the two Membre types, alongside [[Agent]].
...
**Accès (module)**:
The permission model of a web membership in an [[Organisation]], granted **per [[Module Tolbi]]** (not org-wide) and now **carried by the [[Rôle]]**, not set member-by-member: a [[Collaborateur]]'s access to every module is whatever their Rôle says. [[Agent]]s hold a **binary** variant of it: direct per-module grants (no niveau, no [[Rôle]] indirection — « un peu comme un rôle natif ») that gate their **mobile** capabilities (e.g. INA: record transactions); [[Affectation]] stays the fine work-routing axis inside Source/Data OS and **requires** the module grant. For each module a Rôle covers, it grants exactly one *niveau d'accès* — **Lecteur** (read/observe only) or **Éditeur** (create + modify inside that module). The former third niveau **module-Admin** (full control within one module) is **retired**: « Admin » now only names a [[Rôle natif]], and full access everywhere is what the rôles natifs grant. Rendered as one badge per module (module identified by its icon, level by label + colour: Lecteur green, Éditeur amber). A module the Organisation's plan doesn't include renders disabled in the access picker. This *replaces* the earlier single org-wide "Rôle" level (Propriétaire/Admin/Responsable ops/Lecteur as one flat rank per membership).
_Avoid_: rôle as a synonym of *niveau* (the [[Rôle]] is the named bundle that carries this access; Lecteur/Éditeur are the niveaux); permission, droit; Admin as a niveau (retired — it is a [[Rôle natif]] name now)

**Rôle**:
Le paquet **nommé** de permissions d'un [[Collaborateur]] dans une [[Organisation]] : la map module → niveau ([[Accès (module)]]) vit sur le Rôle, et chaque Collaborateur porte **exactement un** Rôle par appartenance, choisi dès l'invitation (champ requis — il n'existe aucun état « sans rôle »). Le Rôle est la **seule** source d'accès d'un membre — pas d'ajustement module par module au niveau du membre ; changer l'accès de quelqu'un = changer son Rôle, ou éditer un [[Rôle custom]] (ce qui impacte tous ses porteurs). Deux familles dans une même liste : les **[[Rôle natif]]s** (livrés avec le produit, non modifiables ni supprimables) et les **[[Rôle custom]]**s (configurés par l'Organisation). Réintroduit le mot « rôle » retiré à l'époque d'ADR-0010 sans en renverser le fond : l'accès reste par module ; il est désormais nommé et réutilisable au lieu d'être posé membre par membre. Les Rôles se gèrent depuis [[Paramètres]] > Organisation : une bascule **Rôles · Membres** (Rôles en vue d'arrivée) dans une barre d'outils unique dont la recherche et l'action contextuelle changent avec la vue ; une seule liste mêle natifs et customs avec leur nombre de porteurs. Dans la table Membres, la colonne Accès d'un [[Collaborateur]] affiche le badge de son rôle (le détail par module vit sur la surface Rôles).
_Avoid_: « rôle » pour un *niveau* d'accès (Lecteur/Éditeur sont des niveaux portés par le Rôle, pas des rôles) ; l'intitulé de poste libre du modèle agent legacy (`Agent.role`) ; profil, groupe

**Rôle natif**:
A [[Rôle]] shipped with the product — non-editable, non-deletable, present in every [[Organisation]]. Three rungs, each with a one-sentence-explainable distinct power: **[[Propriétaire]]** (everything; one per org, transferable), **Super-admin** (manages membres + rôles custom + agents, full access on every module; no billing, no org deletion/transfer), **Admin** (full access on every module; no team management, no billing). All three implicitly cover future modules (full access everywhere), where a [[Rôle custom]] freezes an explicit module map.
_Avoid_: rôle système; rôle par défaut (UI copy for the badge to be settled); Admin-the-rôle vs Admin-the-niveau confusion (say « rôle natif Admin » vs « niveau Admin » when ambiguous)

**Rôle custom**:
Un [[Rôle]] créé par l'[[Organisation]] elle-même (par opposition aux [[Rôle natif]]s) : un **nom** + la map module → niveau (Lecteur/Éditeur) d'[[Accès (module)]] — et **rien d'autre** : jamais de pouvoir d'organisation (gérer membres, rôles, agents reste l'exclusivité de [[Propriétaire]] et Super-admin). Créé, édité et supprimé par le Propriétaire ou un Super-admin ; attribuable à n'importe quel [[Collaborateur]]. L'éditer impacte immédiatement tous ses porteurs ; le supprimer alors qu'il est porté exige de choisir le rôle de destination des porteurs **dans le même geste** (dialogue de suppression — jamais de fallback silencieux).
_Avoid_: rôle personnalisé (copy UI à trancher), template, preset

**Affectation** (EN: Assignment):
Routes mobile collection work to an [[Agent]]: which [[Formulaire (Data OS)]]s (Data OS) and which workflow **steps** ([[TOLBI Source]]) show up in that agent's mobile app. A **separate axis from [[Accès (module)]]** — an Agent's binary module grants gate what their mobile app *can* do; Affectation routes *which* work reaches them within Source/Data OS, **requires** the corresponding module grant, and never shows in the Membres "Accès" column (that column shows the module grants). An Agent is affecté **from inside the module** (at a Source workflow step, or at a Data OS Formulaire), always by **selecting an existing agent from the org roster** — agents are never created there (creation stays in [[Paramètres]] > Organisation). Org-wide reusable: the same Agent can be affecté to many Formulaires and steps across modules.
...
**Propriétaire**:
The top [[Rôle natif]] of an [[Organisation]] — its creator receives it automatically at org creation. **One per org, transferable** (« créateur de l'organisation » is its description, not its name — the rôle stays Propriétaire after a transfer). Sole holder of the existential powers: billing (Abonnement + Paiement) and deleting/transferring the Organisation. Also manages [[Membre]]s, [[Rôle custom]]s and [[Agent]]s (a power it shares with [[Super-admin]]) and holds full [[Accès (module)]] on every module. Gates the Abonnement/Paiement tabs in [[Paramètres]].
_Avoid_: owner (UI copy is Propriétaire); Créateur (description, not the rôle name); conflating with the native rôle [[Rôle natif|Admin]] or the per-module niveau Admin
...
The identity-**credential** module, and a new entry in the Tolbi OS catalogue (not one of the original 11). It takes the [[Producteur]]s already registered in [[TOLBI ID]] and gives them an **added capacity** — a digital agricultural identity: a **code INA** (the [[Numéro INA]]) + an **INA status** that is **activated from the module**. INA does not create producteurs, it credentials them. The INA identity is **permanent / indeterminate**; the [[Carte INA]] is the **determinate**, finite artefact (concrete states, revocable, destructible). Issues enrolled [[Producteur]]s an [[Identité INA]] — a [[Numéro INA]], a physical [[Carte INA]] (opaque QR token + [[Serial de carte]]) and a [[Wallet TOLBI]] — that proves their identity in the field, online or offline, and unlocks a traceable transaction registry. Distinct from [[TOLBI ID]]: ID owns *who the producteur is* (profil + [[Parcelle]]s), INA owns *the verifiable credential and the money/traceability layer* on top, referencing the Producteur by id. Does **not** introduce a competing identity — the [[Producteur]] stays the identity, INA credentials it, and a Producteur may have **no** INA at all.
